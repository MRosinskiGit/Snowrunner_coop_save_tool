...
def swap_keys(external, local, external_id, local_id, _id=404, mirror=False):
    if not mirror:
        # gamestats
        external[external_id]["SslValue"]["gameStat"] = local[local_id]["SslValue"][
            "gameStat"
        ]
        ...
        for key, value in external[external_id]["SslValue"]["garagesData"].items():
            if key in local[local_id]["SslValue"]["garagesData"].keys():
                external[external_id]["SslValue"]["garagesData"][key] = local[local_id][
                    "SslValue"
                ]["garagesData"][key]
            else:
                default = dict(
                    ...
                    slotsDatas=dict(
                        garage_interior_slot_1=dict(
                            garageSlotZoneId="garage_interior_slot_1", truckDesc=None
                        ),
                        garage_interior_slot_2=dict(
                            garageSlotZoneId="garage_interior_slot_2", truckDesc=None
                        ),
                        garage_interior_slot_3=dict(
                            garageSlotZoneId="garage_interior_slot_3", truckDesc=None
                        ),
                        garage_interior_slot_4=dict(
                            garageSlotZoneId="garage_interior_slot_4", truckDesc=None
                        ),
                        garage_interior_slot_5=dict(
                            garageSlotZoneId="garage_interior_slot_5", truckDesc=None
                        ),
                        garage_interior_slot_6=dict(
                            garageSlotZoneId="garage_interior_slot_6", truckDesc=None
                        ),
                    ),
                )
        ...
        if not local[local_id]["SslValue"]["persistentProfileData"]:
            external[external_id]["SslValue"]["persistentProfileData"][
                "ownedTrucks"
            ] = []
            ...
            for key in keys:
                try:
                    external[external_id]["SslValue"]["persistentProfileData"][key] = (
                        local[local_id]["SslValue"]["persistentProfileData"][key]
                    )
                except:
                ...
                if isinstance(dct, dict):
                    for item_name, local_val in dct.items():
                        try:
                            if (
                                local_val
                                and not external[external_id]["SslValue"][
                                    "persistentProfileData"
                                ][key][item_name]
                            ):
                                external[external_id]["SslValue"][
                                    "persistentProfileData"
                                ][key][item_name] = local_val
    ...
    return (
        len(
            [
                ...
                if os.path.isfile(os.path.join(d, f)) and "CompleteSave" in f
            ]
        )
        > 0
    )
...
def get_save_dirs(save_dir, typ="", backup=False):
    ...
    if backup:
        ...
        base_backup_path = os.path.join(
            os.path.dirname(save_dir[0]), "backups", save_dir[1]
        )
...
def get_last_id(target_dir):
    ...
    files = [
        ...
        if os.path.isfile(os.path.join(target_dir, fname)) and "CompleteSave" in fname
    ]
    ids = []
    for f in files:
...
def copy_files(external_dir, local_dir, save_save=False):
    files = [
        ...
        if os.path.isfile(os.path.join(external_dir, f))
        and file_to_copy(f)
        and not file_to_persist(f, save_save=False)
    ]
    removed = 0
...
def get_completesave_info(path, name, show_trucks=True):
    ...
    key = name.split(".")[0]
    try:
        info["discoveredObjectives"] = len(
            save_info[key]["SslValue"]["discoveredObjectives"]
        )
    except:
        info["discoveredObjectives"] = 0

    try:
        info["viewedUnactivatedObjectives"] = len(
            save_info[key]["SslValue"]["viewedUnactivatedObjectives"]
        )
    except:
    ...
    if show_trucks:
        try:
            info["ownedTrucks"] = save_info[key]["SslValue"]["persistentProfileData"][
                "trucksInWarehouse"
            ]
        except:
            info["ownedTrucks"] = []
...
def get_input_completesave(external_dir, msg="??", show_trucks=True):
    _files = [
        f
        for f in os.listdir(external_dir)
        if os.path.isfile(os.path.join(external_dir, f)) and "CompleteSave" in f
    ]
    if len(_files) > 1:
    ...
    if len(files) > 1:
        ...
        for i, f in enumerate(list(files)):
            ...
            if f and ppd:
                saveinfo = get_completesave_info(
                    os.path.join(external_dir, f), f, show_trucks
                )
...
def load_save(mirror=False):
    ...
    external_dir = get_save_dirs(
        os.path.join(application_path, "saves"), typ="external"
    )
    if not external_dir:
        return
    ...
    external_input_save_path, external_input_save_fname = get_input_completesave(
        external_dir,
        msg="Which external save to load map progress from?: ",
        show_trucks=False,
    )
    ...
    files = [
        ...
        if os.path.isfile(os.path.join(local_dir, f)) and "CompleteSave" in f
    ]

    if len(files) > 1 and CONFIG["isolate_save"] == None:
        print(
            f'Remove your previous saves from current save profile?: {green("(your previous saves will remain in backup folder)")}\n'
        )
        ...
        if not save_save:
            print(
                yellow(
                    "Only one save slot will remain in Snowrunner files.",
                    style=style.NORMAL,
                )
            )
        print("")
    ...
    local_input_save_path, local_input_save_fname = get_input_completesave(
        local_dir,
        msg="Which local save to load your vehicles from?: ",
        show_trucks=True,
    )  # FILE TO LOAD VEHICLES FROM
    if not local_input_save_path:
        return

    print(green("Loading vehicles from " + local_input_save_path))

    local_output_save_path, local_output_save_fname = (
        os.path.join(local_dir, external_input_save_fname),
        external_input_save_fname,
    )
    ...
    if os.path.isfile(local_output_save_path):
        with open(local_output_save_path, "r") as f:
            try:
                ...
                ow = json.loads(ow)
                _id = int(
                    ow[local_output_save_fname.split(".")[0]]["SslValue"]["saveId"]
                )
    ...
    external = swap_keys(
        external,
        local,
        external_input_save_fname.split(".")[0],
        local_input_save_fname.split(".")[0],
        _id=_id,
        mirror=mirror,
    )
        ...
        async with aiohttp.ClientSession() as session:
            ...
            try:
                webhook = discord.Webhook.from_url(
                    whc["url"], adapter=discord.AsyncWebhookAdapter(session)
                )
                await webhook.send(
                    embed=embed,
                    file=discord.File(f, filename=fname + ".zip"),
                    username=whc["name"],
                    avatar_url=whc.get("avatar_url"),
                )
                if whc.get("channel_invite_url"):
                    print(
                        green(f'File has been sent to {whc.get("channel_invite_url")}')
                    )
            except Exception as e:
                print(red(str(e)))
...
def set_save_to_slot(save_path, save_fname, slot_n=4, clone=False):
    ...
    while os.path.isfile(new_path) and save_ppd(new_path, new_fname):
        ...
        if act == "1":
            ...
            slot_n = int(
                input(f"Which slot to clone the save to (free slot numbers: {fs}): ")
            )
            new_key = f'CompleteSave{slot_n-1 if slot_n > 1 else ""}'
            new_fname = new_key + ".dat"
            new_path = save_path.replace(save_fname, new_fname)
...
def clone_save():
    ...
    local_input_save_path, local_input_save_fname = get_input_completesave(
        local_dir[0],
        msg="Which local save to include in export file?: ",
        show_trucks=True,
    )  # FILE TO LOAD VEHICLES FROM
    if not local_input_save_path:
        return
    ...
    new_local_input_save_path, new_local_input_save_fname = set_save_to_slot(
        local_input_save_path, local_input_save_fname, slt, clone=True
    )
...
def export():
    ...
    while not save_done:
        ...
        if any(char in save_name for char in illegal_chars):
            print(
                red(
                    f"File name cannot contain following characters {' '.join(illegal_chars)}."
                )
            )
            continue
        break
    ...
    d_zip = os.path.join(
        application_path,
        "export",
        save_name if not is_default else f"{date_for_file}_{save_name}",
    )
    ...
    files = [
        ...
        and file_to_export(f)
        or f == local_input_save_fname
    ]
    ...
    if x == "1" or not x:
        ...
        embed = discord.Embed(
            ...
            color=discord.Colour.from_rgb(3, 175, 255),
        )
        embed.set_author(
            name=datetime.now().replace(microsecond=0),
            icon_url="https://www.freeiconspng.com/uploads/blue-save-disk-icon-0.png",
        )
...
def get_backup_dirs(save_dir):
    found_dirs = [
        ...
        if os.path.isdir(os.path.join(save_dir, d)) and "snieg_backup" in d
    ]
    found_dirs = list(
        sorted(
            [fname for fname in found_dirs],
            key=lambda f: pathlib.Path(f[0]).stat().st_mtime,
            reverse=True,
        )
    )
    ...
    if not len(found_dirs):
        ...
        t = "\n".join(
            [
                f'   {i+1}. {d[1].split("snieg_backup")[-1][1:].replace("_", " ").replace("-", ".")}'
...
async def download_attachment(attachment, clear_saves):
    ...
    if filename.endswith(".zip"):
        ...
        if clear_saves:
            files = [
                os.path.join(saves_folder, f)
                for f in os.listdir(saves_folder)
                if os.path.isfile(os.path.join(saves_folder, f))
            ]
            dirs = [
                os.path.join(saves_folder, f)
                for f in os.listdir(saves_folder)
                if os.path.isdir(os.path.join(saves_folder, f))
            ]
            for f in files:
                os.remove(f)
            for d in dirs:
...
def download_recent(browse=False):
    ...
    with open(
        os.path.join(application_path, "data", "bot.json"), "r", encoding="utf-8"
    ) as f:
        info = json.load(f)
...
def fix_saves():
    ...
    files = [
        f
        for f in os.listdir(local_dir[0])
        if os.path.isfile(os.path.join(local_dir[0], f)) and "CompleteSave" in f
    ]
    print(files)
    for f in files:
...
if __name__ == "__main__":
    ...
    while True:
        try:
            ...
            print(
                fix_highlight(
                    f"   7. Fix invisible saves",
                    style=style.DIM if fix_highlight == white else style.BRIGHT,
                )
            )
            print(white(f"   8. Reload snieg", style=style.DIM))
            print(
                f'   9. Isolate save: {green("Yes") if CONFIG["isolate_save"] else red("No")}'
            )
            print(
                f'   0. Console colors: {green("Yes") if get_colorize() else red("No")}'
            )
            fix_highlight = white
            ...
            if CONFIG["custom_path"]:
                print(
                    white(
                        f'  Custom SnowRunner folder path: {CONFIG["custom_path"]}',
                        style=style.DIM,
                    )
                )
            x = input("\nChoose action: ")
            ...
            if x == "3":
                ...
                input(
                    bg_white(
                        "Finish action... (Press ENTER)", pad=True, style=style.DIM
                    )
                )
            elif x == "9":
                toggle_isolate()
